...
For each crime type, we are going to fit 3 models initially, where each $\beta$ will be the coefficient for which we will derive a posterior distribution for. All models will be fit with a negative binomial sampling model as motivated by the EDA; and all models will be fit with a Cauchy prior with a location (or center) parameter of 0 and a scale of 2.5, in which the intercept and regression coefficients will be further autoscaled by the `stan_glm()` function for further standardization. This prior is proposed by the Gelman paper. 
...
Model 2:
$$
log(mean(stops)) = \beta_0+\beta_1*eth + \beta_2 *pop\_prop + \beta_3*past.arrests + \beta_4 *(eth:past.arrests) +\beta_5*(eth:pop\_prop) + \beta_6*(pop\_prop:past.arrests)
$$
...
From the above 3 models, we will use both the looIC and Bayes' Factor diagnostic to determine which base model has the most predictive power. 
...
model.2 <- stan_glm(data = violent_crime,
                    formula = stops ~ eth * (past.arrests + pop_prop) +
                      pop_prop * past.arrests,
                    family = neg_binomial_2(link = "log"),
                    seed = 360,
                    prior = cauchy(0, 2.5, autoscale = TRUE), 
                    prior_intercept = cauchy(0, 2.5, autoscale = TRUE),
...
```
```{r}
bayesfactor_models(model.1, model.2, model.3, denominator = model.1)
```

Clearly model.1 yielded the best result above as we see that the elpd_diff is negative for both model.2 and model.3, and that the Bayes' Factor has little evidence to suggest that model.2 or model.3 are indeed better than model.1 in terms of posterior predictive performance. The small se_diff compiled with our small sample size of 225 may indicate that the performance difference between those three models may not differ too drastically; however, we can still reasonably claim that model.1 is better.

```{r}
model.4 <- stan_glm(data = violent_crime,
                    formula = stops ~ eth * pop_prop,
...
```{r}
rstanarm::loo_compare(loo(model.1), loo(model.4))
```
```{r}
bayesfactor_models(model.1, model.4, denominator = model.1) #??model.4 is better
```
...
```{r}
model.4 <- stan_glm(data =  weapon_crime,
                    formula = stops ~ eth * pop_prop,
                    family = poisson(link = "log"),
                    offset = log(past.arrests),
                    seed = 360,
...
```
```{r}
bayesfactor_models(model.1, model.4, denominator = model.1)
```
Note that our base model model 1 contains a formula in which all covariates were deemed significant in the EDA, therefore no further comparison needs to be made. 
...
```{r}
model.2 <- stan_glm(data =  property_crime,
                    formula = stops ~ eth * (past.arrests + pop_prop) +
                      pop_prop * past.arrests,
                    family = neg_binomial_2(link = "log"),
                    seed = 360,
...
```
```{r}
bayesfactor_models(model.2, model.3, denominator = model.3) #model 2 is better??
```
...
model.4 <- stan_glm(data = property_crime,
                    ...
                    family = poisson(link = "log"),
                    prior = cauchy(0, 2.5, autoscale = TRUE), 
                    #(if use autoscale, does not converge)
                    prior_intercept = cauchy(0, 2.5, autoscale = TRUE),
                    seed = 360,
                    refresh = 0,
                   diagnostic_file = file.path(tempdir(), "glm4.csv"))
...
```{r}
rstanarm::loo_compare(loo(model.3), loo(model.4))
```

```{r}
bayesfactor_models(model.3, model.4, denominator = model.3) #model 4 is better?
```

```{r}
model.5 <- stan_glm(data = property_crime,
                    formula = stops ~ eth * (crime_prop + pop_prop),
                    family = neg_binomial_2(link = "log"),
                    prior = cauchy(0, 2.5, autoscale = TRUE),
                    prior_intercept = cauchy(0, 2.5, autoscale = TRUE),
...
```{r}
rstanarm::loo_compare(loo(model.3), loo(model.5))
```
```{r}
bayesfactor_models(model.3, model.5, denominator = model.3) #model 5 is better?
```

```{r}
property_model <- model.3
```
...
```
```{r}
bayesfactor_models(model.3, model.1, model.2, denominator = model.3)
```

Clearly model.3 yielded the best result above as we see that the elpd_diff is negative for both model.1 and model.2, and that the Bayes' Factor has little evidence to suggest that model.1 or model.2 are indeed better than model.3 in terms of posterior predictive performance. The small se_diff compiled with our small sample size of 225 may indicate that the performance difference between those three models may not differ too drastically; however, we can still reasonably claim that model.3 is better.
...
```{r}
model.5 <- stan_glm(data = drug_crime,
                    formula = stops ~ eth * (crime_prop + pop_prop),
                    family = neg_binomial_2(link = "log"),
                    prior = cauchy(0, 2.5, autoscale = TRUE),
                    prior_intercept = cauchy(0, 2.5, autoscale = TRUE),
