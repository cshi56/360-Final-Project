...
#apparently all the code below cause errors so I can't even check... Already tried with different prior and starting with different vars. 

#stan.glm.6 <- update(stan.glm.1, formula = stops ~ past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth + past.arrests:pop_prop)
#stan.glm.7 <- update(stan.glm.1, formula = stops ~ crime_rate+ pop+ eth+ crime_rate:eth+ pop:eth +crime_rate:pop)
#stan.glm.8 <- update(stan.glm.1, formula = stops ~ past.arrests+ pop+ eth+ past.arrests:eth+ pop:eth + past.arrests:pop)
```
