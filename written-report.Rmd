...
stan.glm.6 <- stan_glm(data = stop.1,
                   formula = stops ~ past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth + past.arrests:pop_prop,
                   family = neg_binomial_2, #how do I justify not having a prior? 
                   seed = 360,
                   refresh = 0)
stan.glm.7 <- update(stan.glm.6, formula = stops ~ crime_rate+ pop+ eth+ crime_rate:eth+ pop:eth +crime_rate:pop)
stan.glm.8 <- update(stan.glm.6, formula = stops ~ past.arrests+ pop+ eth+ past.arrests:eth+ pop:eth + past.arrests:pop)
stan.glm.9 <- update(stan.glm.6, formula = stops~past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth)
```
...
loo6 <- loo(stan.glm.6, save_psis = TRUE)
loo7 <- loo(stan.glm.7, save_psis = TRUE)
loo8 <- loo(stan.glm.8, save_psis = TRUE)
```
```{r}
loo9 <- loo(stan.glm.9, save_psis = TRUE)
```
```{r}
rstanarm::loo_compare(loo1, loo2, loo3, loo4, loo5, loo6, loo7, loo8,loo9) #loo6 is the best. 
```
```{r}
...
```
```{r}
glm.matrix <- as.matrix(stan.glm.1)
```
```{r}
pp_check(stan.glm.6, plotfun = "stat_2d", stat = c("mean", "sd"))
pp_check(stan.glm.6, plotfun = "scatter_avg")
```

```{r}
#individual model diagnostics - stan.glm.6
y.postpred <- posterior_predict(stan.glm.6)
color_scheme_set("brightblue")
ppc_dens_overlay(stop.1$stops, y.postpred) + xlim(0, 1000) #xlim() truncates so that we focus on the part where x is less than 1000 
```
...
```{r}
stan.glm.1 <- stan_glm(data = stop.2,
                   formula = stops ~ crime_rate+ pop_prop+ eth+ crime_rate:eth+ pop_prop:eth + crime_rate:pop_prop,
                   family = neg_binomial_2,
                   seed = 360, 
                   prior = cauchy(0, 2.5), 
...
```{r}
#checking if adding/changing stuff will make the model better
stan.glm.6 <- stan_glm(data = stop.2,
                   formula = stops ~ past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth + past.arrests:pop_prop,
                   family = neg_binomial_2, #how do I justify not having a prior? 
                   seed = 360,
                   refresh = 0)
...
```
```{r}
rstanarm::loo_compare(loo1, loo2, loo3, loo4, loo5, loo6, loo7, loo8,loo9) 
```

#### Property Crimes
```{r}
stan.glm.1 <- stan_glm(data = stop.3,
                   formula = stops ~ crime_rate+ pop_prop+ eth+ crime_rate:eth+ pop_prop:eth + crime_rate:pop_prop,
                   family = neg_binomial_2,
                   seed = 360, 
                   prior = cauchy(0, 2.5), 
...
```{r}
#checking if adding/changing stuff will make the model better
stan.glm.6 <- stan_glm(data = stop.3,
                   formula = stops ~ past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth + past.arrests:pop_prop,
                   family = neg_binomial_2, #how do I justify not having a prior? 
                   seed = 360,
                   refresh = 0)
...
```{r}
stan.glm.1 <- stan_glm(data = stop.4,
                   formula = stops ~ crime_rate+ pop_prop+ eth+ crime_rate:eth+ pop_prop:eth + crime_rate:pop_prop,
                   family = neg_binomial_2,
                   seed = 360, 
                   prior = cauchy(0, 2.5), 
...
```{r}
#checking if adding/changing stuff will make the model better
stan.glm.6 <- stan_glm(data = stop.4,
                   formula = stops ~ past.arrests+ pop_prop+ eth+ past.arrests:eth+ pop_prop:eth + past.arrests:pop_prop,
                   family = neg_binomial_2, #how do I justify not having a prior? 
                   seed = 360,
                   refresh = 0)
